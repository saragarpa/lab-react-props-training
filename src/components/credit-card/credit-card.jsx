...
function CreditCard({ className = "", card, color, bgColor }) {
  ...
  const dots = (
    <span dangerouslySetInnerHTML={{ __html: "&bull;&bull;&bull;&bull;" }} />
  );
  return (
    <div
      className={`d-flex rounded flex-column p-3 credit-card ${className}`}
      style={{
    ...
    >
      ...
      <img
        ...
        src={type.toLowerCase() === "visa" ? visaLogo : mCardLogo}
        alt={type}
      />
      {/* NUMBER */}
      ...
      <div className="d-flex gap-3 fw-lighter fs-xs">
        <span>
          Expires {expirationMonth}/{`${expirationYear}`.substr(2)}
        </span>
        <span>{bank}</span>
      </div>
      {/* OWNER*/}
